...
export class AddOutletSettlementApprovals1784332800000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`
      CREATE TABLE IF NOT EXISTS "outlet_settlement_approvals" (
        ...
        CONSTRAINT "fk_outlet_settlement_approvals_approved_by" FOREIGN KEY ("approved_by") REFERENCES "users"("id") ON DELETE RESTRICT
      )
    `);
    await queryRunner.query(`
      CREATE INDEX IF NOT EXISTS "ix_outlet_settlement_approvals_outlet"
      ON "outlet_settlement_approvals" ("outlet_id", "approved_at")
    `);
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`DROP INDEX IF EXISTS "ix_outlet_settlement_approvals_outlet"`);
    await queryRunner.query(`DROP TABLE IF EXISTS "outlet_settlement_approvals"`);
  }
}
