...
## Staging2 environment

Create `staging2` as another Dokploy Compose service, not by editing the
development or staging services in place. It should use the same
`deploy/dokploy/compose.yaml`; setting `DEPLOY_ENV=staging2` gives it separate
Compose service names, network, PostgreSQL volume, and Redis volume.

Create or select a Dokploy project environment named `staging2`, then create a
Docker Compose service named `rsc-web-staging2` with:
...
Paste the contents of `deploy/dokploy/staging2.env.example` into the staging2
service's **Environment** tab, replacing every placeholder. Generate new values
for staging2 rather than copying development or staging secrets:
...
In Namecheap **Advanced DNS**, add these records. They point to the same Dokploy
server; Traefik routes each hostname to the correct staging2 container.

| Type     | Host                    | Value          | TTL       |
| -------- | ----------------------- | -------------- | --------- |
| A Record | `staging2`              | `72.61.202.26` | Automatic |
| A Record | `admin-staging2`        | `72.61.202.26` | Automatic |
| A Record | `api-staging2`          | `72.61.202.26` | Automatic |
| A Record | `outlet-admin-staging2` | `72.61.202.26` | Automatic |
...
In the staging2 Compose service's **Domains** tab, add:

| Service         | Domain                             | Port   |
| --------------- | ---------------------------------- | ------ |
| `customer-web`  | `staging2.rscapp.xyz`              | `3000` |
| `central-admin` | `admin-staging2.rscapp.xyz`        | `8080` |
| `api`           | `api-staging2.rscapp.xyz`          | `4000` |
| `outlet-admin`  | `outlet-admin-staging2.rscapp.xyz` | `8080` |

For every domain use external path `/`, internal path `/`, **Strip path Off**,
HTTPS **On**, and a Let's Encrypt certificate. Save the domains, then redeploy
the staging2 Compose service so Dokploy applies the routing configuration.

Verify DNS before requesting certificates:

```bash
dig +short staging2.rscapp.xyz
dig +short admin-staging2.rscapp.xyz
dig +short api-staging2.rscapp.xyz
dig +short outlet-admin-staging2.rscapp.xyz
```

All four commands must return `72.61.202.26`. After deployment, verify:

```text
https://staging2.rscapp.xyz
https://admin-staging2.rscapp.xyz
https://api-staging2.rscapp.xyz/api/v1/health/live
https://api-staging2.rscapp.xyz/api/v1/health/ready
https://api-staging2.rscapp.xyz/api/docs
https://outlet-admin-staging2.rscapp.xyz
```

In **Containers**, staging2 names should begin with `rsc-web-staging2`. In
**Volumes**, confirm staging2 has separate PostgreSQL and Redis volumes before
creating test customer data. Configure a staging2 PostgreSQL volume backup on a
different schedule from development and staging.
